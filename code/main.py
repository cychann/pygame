...
def collision(obj1, obj2):
    ...
    if lastcollision is not None and time.time() - lastcollision < 3:# 마지막 충돌 시간과 현재 시간을 비교해서 그 차이가 무적 시간보다 작다면 충돌을 감지하지 못하도록
        if ((int(((time.time()-lastcollision))*10))%2 == 0):    #일정시간동안 깜빡이게 하기 위해 0과 1의 값이 반복되서 나오도록
            player.nodamgetime = True   #위 조건을 만족할때 True를 넣어주면 이 시간에는 비행기가 출력이 안된다.
        else:
            player.nodamgetime = False  #위 조건을 만족하지 않을 때는 False값을 넣어줘서 비행기가 출력된다.
            
            
        return False
    if math.sqrt((obj1.pos[0] - obj2.pos[0]) ** 2 + (obj1.pos[1] - obj2.pos[1]) ** 2) < 20:
        collision_sound.play() #부딪히면 효과음(미션1)
        screen.blit(boom_image, (obj1.pos[0], obj1.pos[1])) #부딪혔을 때 터지는 그림효과(미션2)
...
bullets1 = [] #총알1
for i in range(3):
    bullets1.append(Bullet1(0, rnd.random()*HEIGHT, rnd.random()-0.5, rnd.random()-0.5))
...
while running:
    ...
    if gameover:
        ...
        txt = "Time: {:.1f}  Bullets: {}".format(score, len(bullets1)+ len(bullets2) + len(bullets3))#총알1+총알2+총알3
        ...
        for s in scorelist:
            tmp += 1 
            draw_text("({}) : {}".format(tmp, s.strip()), 20, (50, 30*tmp), (255,255,255)) #30*tmp를 하여 각 리스트를 출력할때마다 각각 다른 줄에 입력되도록
        if len(scorelist) < 10: #만약 점수가 10개보다 적게 저장되어 있으면 매기록이 10위 안에 드므로 무조건 ranking안에 존재한다.
            draw_text("New record!! ->", 32, (100, 450), (0,255,0)) #시간점수 옆에 메시지를 띄어주고
            draw_text("! Congratulations !", 70, (230, 100), (0,255,0)) #축하 메시지와
            draw_text("It's in the ranking score!!", 32, (330, 500), (0,255,0)) #점수가 순위권 안에 있음을 강조한다.
        elif len(scorelist) >= 10: #만약 점수가 10개이상이 되면, 점수표에 기록되어 있는 점수 중에서 가장 작은 수와 현재 스코어를 비교한다.
            if score > float(scorelist[9]): #만약 현재 스코어가 순위권 안에 있다면
                ...
                draw_text("It's in the ranking score!!", 32, (330, 500), (0,255,0)) 
            elif score <= float(scorelist[9]):  #만약 현재 스코어가 순위권 안에 들지 못하는 점수라면
                draw_text("Try Again..!", 70, (330, 100), (255,0,0))    #다시 시도하여 순위권에 들라는 메시지를 보여준다
    else:
        score = time.time() - start_time
        txt = "Time: {:.1f}  Bullets: {}".format(score, len(bullets1) + len(bullets2) + len(bullets3))#총알1+총알2+총알3
        draw_text(txt, 32, (10, 10), (255,255,255))
        lifetxt = "life: %d" % (player.life) 
        draw_text(lifetxt, 32, (WIDTH/2, 10), (255,255,255)) #게임 상단에 life를 정수로 표현

        for life in range(player.life):
            screen.blit(life_image, (life*30 + 600 ,10)) #목숨 개수 만큼 오른쪽 상단에 life이미지 출력
    ...
    if not gameover:
        ...
        if time_for_adding_bullets > 1000:
            bullets1.append(Bullet1(0, rnd.random()*HEIGHT, rnd.random()-0.5, rnd.random()-0.5))
            bullets2.append(Bullet2(0, rnd.random()*HEIGHT, rnd.random()-0.5, rnd.random()-0.5))
            bullets3.append(Bullet2(0, rnd.random()*HEIGHT, rnd.random()-0.5, rnd.random()-0.5))
